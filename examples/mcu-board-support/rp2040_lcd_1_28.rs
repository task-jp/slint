...
use waveshare_rp2040_lcd_1_28::hal::gpio::{self, Interrupt as GpioInterrupt};
...
// 8ns for serial clock cycle (write), https://files.waveshare.com/upload/5/5e/GC9A01A.pdf
const SPI_GC9A01A_MAX_FREQ: Hertz<u32> = Hertz::<u32>::Hz(8_000_000);
...
pub fn init() {
    ...
    display.clear(pixelcolor::Rgb565::CSS_FOREST_GREEN).unwrap();
    // Turn on backlight
...
impl<
    ...
    for WaveshareRp2040LCD1_28Backend<
        DrawBuffer<gc9a01a::GC9A01A<DI, RST, PWM>, PioTransfer<TO, CH>, (DC_, CS_)>,
    >
{
...
impl<
    ...
    for &mut DrawBuffer<gc9a01a::GC9A01A<DI, RST, PWM>, PioTransfer<TO, CH>, (DC_, CS_)>
{
    ...
    fn process_line(
        ...
        self.display.clear(pixelcolor::Rgb565::CSS_FOREST_GREEN).unwrap();
...
fn panic(info: &core::panic::PanicInfo) -> ! {
    ...
    let sio = hal::sio::Sio::new(pac.SIO);
    let pins = waveshare_rp2040_lcd_1_28::Pins::new(pac.IO_BANK0, pac.PADS_BANK0, sio.gpio_bank0, &mut pac.RESETS);
